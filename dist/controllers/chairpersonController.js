...
const chairpersonController = express_1.Router();
/********************************
    ADD Chairperson
 *******************************/
chairpersonController.post("/addChairperson", async (req, res) => {
    ...
    try {
        const clubMember = await models_1.User.findOne({ where: { email: memberEmail } }); //Find userId for new clubMember by email address.
        const clubRole = await models_1.ClubRoster.findOne({
...
chairpersonController.put("/updateClub", async (req, res) => {
    ...
    try {
        const club = await models_1.Club.findOne({ where: { id: clubId } });
...
chairpersonController.delete("/removeClub", async (req, res) => {
    ...
    try {
        const club = await models_1.Club.findOne({
            where: { id: clubId },
        });
        await club.destroy();
        res.status(200).json({
...
exports.default = chairpersonController;
