const { ClubRoster, User, Club, Activity } = require("../models");
const { Router } = require("express");
const sequelize = require("../db");
const viceChairController = Router();
...
viceChairController.post("/addAthlete", async (req, res) => {
  ...
  try {
    const athlete = await User.findOne({ where: { email: clubAthleteEmail } }); //Find userId for new club member by email address.
    if (athlete) {
...
module.exports = viceChairController;
